...
- If the user asks to enrich, deepen, refresh sources, update knowledge, improve an active pack, or fill gaps in active knowledge, route to `knowledge\knowledge-enrichment-workflow.md`. Do not overwrite active knowledge blindly; create a proposed enrichment with evidence, update `enrichment-history.md`, and preserve the active baseline until the enrichment is reviewed/applied.
- Candidate knowledge remains staged and not trusted as an implementation rule. Enrichment candidates are proposed improvements, not automatic rewrites.
...
Headroom/context-compression routing: headroom, context compression, token compression, compress tool outputs, compress logs, reversible compression, CCR, cross-agent memory, headroom learn, wrap Codex, proxy compression, or MCP compression -> active SDK pilot pack `project_manager.context.headroom-compression-evaluation`. Run `pm-headroom-status.mjs` first. Use `pm-headroom-context.mjs` for explicit context analysis and for service-backed simulate/compress only when service health is green. Do not configure MCP/proxy, wrap Codex, or run `headroom learn` without safety review, telemetry decision, generated-file plan, and rollback plan.
