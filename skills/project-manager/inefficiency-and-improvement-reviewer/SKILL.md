...
When Headroom/context-compression requests skip `project_manager.context.headroom-compression-evaluation`, `pm-headroom-status.mjs`, or `pm-headroom-context.mjs`, treat it as a routing inefficiency. Recommend active SDK-pilot lookup first, comparison against existing retrieval/logging practices, and safety-gated planning before any MCP, proxy, wrapper, or `headroom learn` action.
