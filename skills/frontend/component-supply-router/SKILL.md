...
Reuse local components first. Check the component map inside projects. Use shadcn/Radix for primitives and behavior-heavy components. Use Magic UI, Aceternity, Animate, or other registries for visual polish only when suitable. Use 21st.dev Magic only if configured and appropriate. Do not custom-code common primitives if a library exists.
